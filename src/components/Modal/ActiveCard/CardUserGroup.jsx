...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMembers }) {
  ...
  const handleUpdateCardMembers = (user) => {
    ...
    const incomingMemberInfo = {
      userId: user?._id,
      action: cardMemberIds.includes(user?._id) //Nếu user đã là thành viên của card thì sẽ gửi lệnh REMOVE để xóa khỏi card, ngược lại thì gửi lệnh ADD để thêm vào card nghĩa có tick xanh ở trên avatar user đó (user?._id nằm trong cardMemberIds)
          ...
          {board.FE_allUsers.map((user, index) => (
            <Tooltip title={user?.displayName} key={index}>
              ...
              <Badge
                ...
                //khi click vào user thì sẽ gửi thông tin user đó lên BE để thêm hoặc xóa khỏi card thông qua hàm handleUpdateCardMembers
